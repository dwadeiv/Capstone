...
#include  <rtos/cpu/include/cpu_cache.h>
                           ...
                           RTOS_ERR  *p_err)
{
    ...
    Mem_PoolCreate (&p_dev_data->RxDescPool,                    /* Pass a pointer to the mem pool to create.            */
             (void *)p_dev_cfg->MemAddr,                        /* From the dedicated memory.                           */
                     p_dev_cfg->MemSize,                        /* Dedicated area size.                                 */
                     1,                                         /* Allocate 1 block.                                    */
                     nbytes,                                    /* Block size large enough to hold all Rx descriptors.  */
                     32u,                                       /* Block alignment (see Note #8).                       */
                    &reqd_octets,                               /* Optional, ptr to variable to store rem nbr bytes.    */
    ...
    Mem_PoolCreate (&p_dev_data->TxDescPool,                    /* Pass a pointer to the mem pool to create.            */
             (void *)p_dev_cfg->MemAddr,                        /* From the dedicated memory.                           */
                     p_dev_cfg->MemSize,                        /* Dedicated area size.                                 */
                     1,                                         /* Allocate 1 block.                                    */
                                 ...
                                 RTOS_ERR  *p_err)
{
    ...
    p_desc      = (DEV_DESC *)Mem_PoolBlkGet(p_mem_pool,
                                             nbr_octets,
                                             p_err);
    if (RTOS_ERR_CODE_GET(*p_err) != RTOS_ERR_NONE) {
        return;
                                 ...
                                 RTOS_ERR  *p_err)
{
    ...
    nbr_octets =  p_dev_cfg->TxDescNbr * sizeof(DEV_DESC);
    p_desc      = (DEV_DESC *)Mem_PoolBlkGet(p_mem_pool,
                                             nbr_octets,
                                             p_err);
    if (RTOS_ERR_CODE_GET(*p_err) != RTOS_ERR_NONE) {
