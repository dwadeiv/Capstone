...
*
* Note(s)     : (1) "The Timestamp is a right-justified, 32-bit timestamp in milliseconds since midnight
...
*
* Note(s)     : (1) (a) (1) Although RFC #2988, Section 4 states that "there is no requirement for the
...
*
*               (2) [INTERNAL]
*                   (a) To avoid  timestamp calculation overflow, timestamps are updated by continually
...
*
* Description : Get the timestamp value in milliseconds.
*
* Argument(s) : None.
*
* Return(s)   : Current Timestamp value in milliseconds.
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
*
* Description : Get the maximum delta value between two Timestamps to avoid counter overflow.
*               This is based on the maximum value of the TS counter and the TS frequency.
*
* Argument(s) : None.
*
* Return(s)   : Maximum delta value in milliseconds.
*
* Note(s)     : None.
*********************************************************************************************************
*/
                                                ...
                                                CPU_INT16U   hdr_size)
{
    ...
    sum = NetUtil_16BitSumHdrCalc(p_hdr, hdr_size);             /* Calc  16-bit sum (see Note #3a).                     */
...
*
*
* Argument(s) : p_hdr       Pointer to packet header.
*
*               hdr_size    Size of packet header.
*
                                                  ...
                                                  CPU_INT16U   hdr_size)
{
    ...
    sum = NetUtil_16BitSumHdrCalc(p_hdr, hdr_size);             /* Calc 16-bit sum (see Note #4a).                      */
...
*
*
* Argument(s) : p_data_buf          Pointer to packet data network buffer(s) (see Note #2a).
*
*               p_pseudo_hdr        Pointer to transport layer pseudo-header (see Note #2b).
*
...
*
*
* Argument(s) : p_data_buf          Pointer to packet data network buffer(s) (see Note #3a).
*
*               p_pseudo_hdr        Pointer to transport layer pseudo-header (see Note #3b).
*
*               pseudo_hdr_size     Size    of transport layer pseudo-header.
*
* Return(s)   : DEF_OK,   if valid check-sum.
...
#else
    val += (CPU_INT32U)KAL_TickGet(&local_err);
#endif
...
*
*               p_octet_prev        Pointer to last octet from a fragmented packet's previous buffer.
*               -----------         Argument validated in NetUtil_16BitOnesCplSumDataCalc().
*
*               p_octet_last        Pointer to variable that will receive the value of the last octet from a
...
*               ------------        Argument validated in NetUtil_16BitOnesCplSumDataCalc().
*
*               p_sum_err           Pointer to variable that will receive the error return code(s) from this function :
*
...
*                                                                       Checksum'.
*
* Argument(s) : p_data_buf          Pointer to packet data network buffer(s) (see Notes #1a & #2).
*
*               p_pseudo_hdr        Pointer to transport layer pseudo-header (see Note  #1b).
*
*               pseudo_hdr_size     Size    of transport layer pseudo-header.
*
