...
*                                        NetASCII_Str_to_MAC()
*
* Description : Converts an Ethernet MAC address ASCII string to an Ethernet MAC address.
*
* Argument(s) : p_addr_mac_ascii    Pointer to an ASCII string that contains a MAC address.
*
...
*                           addresses ... should be written hyphenated by octets
...
*
* Argument(s) : p_addr_mac          Pointer to a memory buffer that contains the MAC address
*                                   (see Note #2).
*
*               p_addr_mac_ascii    Pointer to an ASCII character array that will receive the return MAC
*
...
*                               MAC Address Examples : "00-1A-07-AC-22-09" = 0x001A07AC2209
*
*                   (b) (1) The return MAC address ASCII string formats EXACTLY six hexadecimal values
...
#else
            RTOS_DBG_FAIL_EXEC_ERR(*p_err, RTOS_ERR_NOT_AVAIL, NET_IP_ADDR_FAMILY_UNKNOWN);
#endif
             break;
...
*
*                       (2) Include UP TO four decimal values separated by UP TO three dot characters
...
*                               IP Address Examples : 192.168.1.64 = 0xC0A80140
*
*                   (b) (1) The return dotted-decimal IPv4 address ASCII string formats EXACTLY four decimal values
...
*
*                   (b) If leading zeros option DISABLED & the decimal value of the octet is zero;
...
*
* Description : Convert a network protocol IPv6 address in host-order into an IPv6 address ASCII string
*                   in dotted-decimal notation.
*
* Argument(s) : p_addr_ip           Pointer to IPv6 address.
*
*               p_addr_ip_ascii     Pointer to an ASCII character array that will receive the return IPv6
*
*               hex_lower_case      DEF_YES, hexadecimal value will be in lower case
*                                   DEF_NO,  otherwise.
*
*               lead_zeros          Prepend leading zeros option (see Note #2) :
*
...
*                                       (see Note #1).
*
*               p_dot_nbr           Pointer to the number of dot found in the ASCII string.
*
*               p_err               Pointer to variable that will receive the return error code from this function.
*
* Return(s)   : Host-order IPv4 address represented by ASCII string, if NO error(s).
                                             ...
                                             RTOS_ERR    *p_err)
{
   ...
   *p_dot_nbr          =  0u;
    ...
    while ((p_char_cur != DEF_NULL) &&                                  /* Parse ALL non-NULL chars in ASCII str.       */
        ...
        switch (*p_char_cur) {
            ...
            case '.':
                 ...
                 if (*p_char_prev == '.') {                             /* If prev char a dot  (see Note #1b2B1), ...   */
                     RTOS_ERR_SET(*p_err, RTOS_ERR_NET_STR_ADDR_INVALID);
                     return(NET_IPv4_ADDR_NONE);
                 }
    ...
    if (*p_char_prev == '.') {                                          /* If last char a dot (see Note #1b2B1), ..     */
        RTOS_ERR_SET(*p_err, RTOS_ERR_NET_STR_ADDR_INVALID);
        return(NET_IPv4_ADDR_NONE);
    }
