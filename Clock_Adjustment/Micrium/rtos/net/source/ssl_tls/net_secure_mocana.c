...
#include  <rtos/net/source/tcpip/net_sock_priv.h>
...
*               DEF_FAIL.
*
* Note(s)     : [INTERNAL] Net_Secure_CA_CertIntall() is called by application function(s) & MUST NOT be called
*               with the global network lock already acquired; MUST block ALL other network protocol tasks by
*               pending on & acquiring the global network lock.
*
                                             ...
                                             RTOS_ERR             *p_err)
{
    ...
    Net_GlobalLockAcquire((void *)&NetSecure_CA_CertIntall);
...
*
* Return(s)   : DEF_OK,   all distinguished name data printed successfully.
*               DEF_FAIL, otherwise.
*
* Note(s)     : none.
*********************************************************************************************************
...
void  NetSecure_Init (RTOS_ERR *p_err)
{
    CPU_INT32S  rc;
    CPU_SR_ALLOC();
...
*
* Argument(s) : p_sock          Pointer to the server's socker to configure certificate and key.
*
*               sock_type       Secure socket type:
*
*                                   NET_SOCK_SECURE_TYPE_SERVER
*                                   NET_SOCK_SECURE_TYPE_CLIENT
*
*               p_buf_cert      Pointer to the certificate buffer to install.
*
*               buf_cert_size   Size of the certificate buffer to install.
*
*               p_buf_key       Pointer to the key buffer to install.
*
