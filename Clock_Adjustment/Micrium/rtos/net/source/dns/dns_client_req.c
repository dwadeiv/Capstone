...
#define  DNSc_SERVER_ADDR_DFLT_STR         "8.8.8.8"
        ...
        if (valid == DEF_YES) {
           *p_addr = server_addr.AddrObj;
            ...
            if (DNSc_DataPtr->CfgDfltServerAddrFallbackEn == DEF_YES) {
                DNScCache_AddrObjSet(p_addr, DNSc_SERVER_ADDR_DFLT_STR, p_err);
...
*                                           DNSc_ReqIF_Sel()
*
* Description : Select next interface for DNS process.
*
* Argument(s) : if_nbr_last     Previous Interface number.
*
*               sock_id         Socket ID to use.
*
*               p_err           Pointer to variable that will receive the return error code from this function.
*
* Return(s)   : New interface number.
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
*
* Argument(s) : p_resp_msg      Pointer to the response's message.
*
                                     ...
                                     RTOS_ERR       *p_err)
{
    ...
    switch (answer_type) {
        case DNSc_TYPE_A:
           ...
           *(CPU_INT32U *)&p_addr->Addr.Array[0] = NET_UTIL_NET_TO_HOST_32(*(CPU_INT32U *)&p_addr->Addr.Array[0]);
