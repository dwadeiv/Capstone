...
*                                         NetIPv4_CfgAddrAdd()
*
* Description : Add a statically-configured IPv4 host address, subnet mask, & default gateway to an interface.
*
* Argument(s) : if_nbr              Interface number to configure.
*
*               addr_host           Desired IPv4 address to add to this interface.
*
*               addr_subnet_mask    Desired IPv4 address subnet mask to configure.
*
*               addr_dflt_gateway   Desired IPv4 default gateway address to configure.
...
*
*               (3) (a) (1) A host on an isolated network should be able to correctly operate
...
*
* Description : Remove a configured IPv4 host address, subnet mask, & default gateway from an interface
*
* Argument(s) : if_nbr      Interface number to remove address configuration.
*
*               addr_host   IPv4 address to remove.
*
*               p_err       Pointer to variable that will receive the return error code from this function :
*
*                               RTOS_ERR_NONE
...
*
* Description : Remove all configured IPv4 host address(s) from an interface.
*
* Argument(s) : if_nbr  Interface number to remove address configuration.
*
*               p_err   Pointer to variable that will receive the return error code from this function :
...
*
* Description : Start the IPv4 Link Local process on the given interface.
*
* Argument(s) : if_nbr  Interface number on which the IPv4 Link Local process will be started.
*
*               hook    Hook function to be notified when the process is completed.
*
...
* Return(s)   : None.
*
* Note(s)     : (1) An IPv4 link local address will only be configured if no other IPv4 addresses are
...
*
* Description : Stop the IPv4 Link Local process on the given interface and remove the link local address
...
*
*               (3) [INTERNAL] 'NetIPv4_FragReasmTimeout_ms' variables MUST ALWAYS be accessed exclusively
...
*                                         NetIPv4_GetAddrHost()
*
* Description : Get an interface's IPv4 host address(s).
*
...
*
*               p_addr_tbl_qty  Pointer to a variable to pass the table size and will return the number of
...
*               address).
*
* Argument(s) : addr_remote     Remote address to get configured IPv4 host address.
*
*               p_err           Pointer to variable that will receive the return error code from this function :
*
*                                   RTOS_ERR_NONE
...
*                                   RTOS_ERR_INVALID_HANDLE
*
* Return(s)   : Configured IPv4 host address, if available.
...
*
* Argument(s) : addr    Configured IPv4 host address to get the subnet mask.
*
*               p_err   Pointer to variable that will receive the return error code from this function :
*
*                           RTOS_ERR_NONE
...
*                   asynchronous to other network protocol tasks.
*
*               (2) IPv4 address returned in host-order.
*********************************************************************************************************
*/
...
*
* Argument(s) : addr    Configured IPv4 host address to get the default gateway.
*
*               p_err   Pointer to variable that will receive the return error code from this function :
*
*                           RTOS_ERR_NONE
...
*               DEF_NO,  otherwise.
*
* Note(s)     : (3) IPv4 address MUST be in host-order.
*********************************************************************************************************
*/
...
*
*
* Argument(s) : addr_host           IPv4 host address to validate.
*
*               addr_subnet_mask    IPv4      address subnet mask.
*
* Return(s)   : DEF_YES, if this host's IPv4 address valid.
*               DEF_NO,  otherwise.
*
* Note(s)     : (3) IPv4 addresses MUST be in host-order.
*********************************************************************************************************
*/
...
*
* Argument(s) : addr_subnet_mask    IPv4 address subnet mask to validate.
*
...
*
* Description : Initialize the IPv4 link local module.
*
* Argument(s) : p_mem_seg   Pointer to memory segment to use for dynamic allocation.
*
*               p_err       Pointer to variable that will receive the return error code from this function.
*
* Return(s)   : None.
...
*
* Description : Add a dynamically-configured IPv4 host address, subnet mask, & default gateway to
*               an interface.
*
* Argument(s) : if_nbr              Interface number to configure.
*
...
*                   asynchronous to other network protocol tasks.
*
*               (2) Application calls to dynamic address configuration functions MUST be sequenced as follows :
*
...
*
* Description : Start the dynamic address configuration for an interface.
*
...
*
* Description : Stop the dynamic address configuration for an interface.
*
...
*
* Argument(s) : if_nbr              $$$$ Add description for 'if_nbr'
*
*               addr_host           $$$$ Add description for 'addr_host'
*
*               addr_subnet_mask    $$$$ Add description for 'addr_subnet_mask'
*
*               addr_dflt_gateway   $$$$ Add description for 'addr_dflt_gateway'
*
...
* Return(s)   : none.
*
* Note(s)     : (3) This function is a network protocol suite to network interface (IF) function & SHOULD be
*                   called only by appropriate network interface function(s).
*
*               (4) Since NetIPv4_RxPktFragReasm() may return a pointer to a different packet buffer (see
*                   'NetIPv4_RxPktFragReasm()  Return(s)', 'p_buf_hdr' MUST be reloaded.
*
*               (5) (a) For single packet buffer IPv4 datagrams, the datagram length is equal to the IPv4
                                          ...
                                          RTOS_ERR           *p_err)
{
    ...
    if (frag != DEF_YES) {                                      /* If pkt NOT a frag, ...                               */
       *p_is_frag = DEF_NO;
        p_frag    = p_buf;
        goto exit;                                              /* ... rtn non-frag'd datagram (see Note #3b1B).        */
    }
    ...
    while (frag_insert_done == DEF_NO) {
        ...
        if (frag_offset > frag_list_cur_frag_offset) {                  /* While frag offset > cur frag offset, ...     */
            ...
            if (frag_list_discard == DEF_NO) {                          /* If frag does NOT overlap, ...                */
                ...
                RTOS_ERR_SET(local_err, RTOS_ERR_NONE);
                NetIPv4_RxPktFragListDiscard(p_frag_list, DEF_YES, &local_err); /* ... (see Note #1b2).                  */
                NET_CTR_ERR_INC(Net_ErrCtrs.IPv4.RxFragDisCtr);
                p_frag = DEF_NULL;
                RTOS_ERR_SET(*p_err, RTOS_ERR_RX);
                goto exit;
                                     ...
                                     RTOS_ERR       *p_err)
{
    ...
    if (p_addr_obj != DEF_NULL) {
        ...
        if (tx_remote_host != DEF_NO) {                             /* If localhost addr tx'd to remote host, ...       */
            NET_CTR_ERR_INC(Net_ErrCtrs.IPv4.TxInvAddrSrcCtr);
            RTOS_ERR_SET(*p_err, RTOS_ERR_TX);                          /* ... rtn err / discard pkt   (see Note #2e2A).    */
            goto exit;
        }
    ...
    if (addr_cfgd == DEF_YES) {
        ...
        if (p_addr_obj != DEF_NULL) {
            if (p_addr_obj->AddrDfltGateway == NET_IPv4_ADDR_NONE) {     /* If dflt gateway NOT cfg'd, ...               */
                RTOS_ERR_SET(*p_err, RTOS_ERR_NET_NEXT_HOP);             /* ... rtn err (see Note #3b1c1).               */
                goto exit;
            }
        }
        ...
        p_buf_hdr->IP_AddrNextRoute = p_addr_obj->AddrDfltGateway;      /* ... via dflt gateway (see Note #3a1B).       */
...
*
* Description : IPv4 link local module's hook function for the interface link change.
*
* Argument(s) : if_nbr      Interface number.
*
*               link_state  Interface link state:
*
*                               NET_IF_LINK_UP
*                               NET_IF_LINK_DOWN
*
...
*
* Argument(s) : p_child     Pointer to the IPv4 Link local module Network Service Task Child.
*
...
* Argument(s) : p_child     Pointer to the IPv4 Link local module Network Service Task Child.
*
*               p_obj       Pointer to current IPv4 link local object.
*
*               p_err       Pointer to variable that will receive the return error code from this function.
*
* Return(s)   : None.
...
*
* Description : Start the IPv4 link local process.
*
* Argument(s) : p_obj   Pointer to the IPv4 link local object.
*
*               p_err   Pointer to variable that will receive the return error code from this function.
*
...
*
* Description : Reboot the IPv4 link local process.
*
* Argument(s) : p_obj   Pointer to the IPv4 link local object.
*
*               p_err   Pointer to variable that will receive the return error code from this function.
...
*
* Description : Stop the IPv4 link local process.
*
* Argument(s) : p_obj   Pointer to the IPv4 link local object.
*
*               p_err   Pointer to variable that will receive the return error code from this function.
...
*
* Description : Create the IPv4 link local address and probe the network for the address.
*
* Argument(s) : p_obj   Pointer to the IPv4 link local object.
*
*               p_err   Pointer to variable that will receive the return error code from this function.
...
*
* Description : Add the IPv4 link local address on the interface address list.
*
* Argument(s) : p_obj   Pointer to the IPv4 link local object.
*
*               p_err   Pointer to variable that will receive the return error code from this function.
...
*
* Description : Announce the new IPv4 link local address that has been configured on the interface.
*
* Argument(s) : p_obj   Pointer to the IPv4 link local object.
*
*               p_err   Pointer to variable that will receive the return error code from this function.
...
*
* Description : Create a IPv4 Link local address from the 169.254.0.0/16 prefix, the harware address and
*               a random parameter.
*
* Argument(s) : p_addr_hw       Pointer to the interface hardware address.
*
                                                 ...
                                                 CPU_INT08U   addr_hw_len)
{
    ...
    time_cur  =  KAL_TickGet(&local_err);                       /* See Note #2b.                                        */
...
*
* Description : Start the IPv4 Link local address configuration process on the given interface.
*
* Argument(s) : if_nbr  Interface number on which to start the IPv4 link local process.
*
*               p_err   Pointer to variable that will receive the return error code from this function.
*
* Return(s)   : DEF_OK, if process started successfully.
*               DEF_FAIl, otherwise.
*
* Note(s)     : None.
*********************************************************************************************************
...
*
* Description : Add the IPv4 Link local address on the given interface.
*
* Argument(s) : if_nbr              Interface number on which to add the Ipv4 link local address.
*
*               addr_host           IPv4 link local address.
*
*               p_err               Pointer to variable that will receive the return error code from this function.
*
* Return(s)   : DEF_OK, if address added successfully.
*               DEF_FAIl, otherwise.
*
* Note(s)     : None.
*********************************************************************************************************
...
*
* Description : Stop the IPv4 Link local address configuration process on the given interface.
*
* Argument(s) : if_nbr  Interface number on which to remove the link local address.
*
*               p_err   Pointer to variable that will receive the return error code from this function.
*
* Return(s)   : DEF_OK, if process stopped successfully.
*               DEF_FAIl, otherwise.
*
* Note(s)     : None.
*********************************************************************************************************
