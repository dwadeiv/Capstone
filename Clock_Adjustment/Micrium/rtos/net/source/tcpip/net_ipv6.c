...
*
*               (2) [INTERNAL] RFC #4862 , Section 4 states that "Autoconfiguration is performed only on
...
*                   asynchronous to other network protocol tasks.
*
*               (2) After disabling, in a case of a link status change, the auto-configuration will
*                   not be called. The Hook function after the Auto-configuration completion will also
*                   not occurred.
*
*               (3) The address previously configured with the auto-configuration will NOT be removed
*                   after disabling.
*********************************************************************************************************
*/
#ifdef  NET_IPv6_ADDR_AUTO_CFG_MODULE_EN
...
*
* Description : Configure the IPv6 Address Configuration hook function. This function will be called
*               each time a IPv6 address has finished being configured.
*
* Argument(s) : fnct    Pointer to hook function to call when the IPv6 static address configuration ends.
*
*               p_err   Pointer to variable that will receive the return error code from this function :
*
*                           RTOS_ERR_NONE
...
*
* Description : Remove subscribe hook function for IPv6 address configuration.
*
* Argument(s) : fnct    Pointer to hook function to remove.
*
* Return(s)   : none.
*
* Note(s)     : (1) [INTERNAL] This function is called by application function(s) :
...
*                                        NetIPv6_CfgAddrAdd()
*
* Description : Add a statically-configured IPv6 host address to an interface.
*
* Argument(s) : if_nbr      Interface number to configure.
*
*               p_addr      Pointer to desired IPv6 address to add to this interface.
*
...
*
* Description : Remove a configured IPv6 host address & multicast solicited mode address from an
*               interface.
*
...
*
* Description : Remove all configured IPv6 host address(s) from an interface.
*
* Argument(s) : if_nbr  Interface number to remove address configuration.
*
*               p_err   Pointer to variable that will receive the return error code from this function :
...
*               DEF_FAIL, otherwise.
*
* Note(s)     : (1) IPv6 fragment reassembly timeout is the maximum time allowed between received IPv6
*                   fragments from the same IPv6 datagram.
*
*               (2) [INTERNAL] 'NetIPv6_FragReasmTimeout_ms' variables MUST ALWAYS be accessed exclusively
...
*
* Argument(s) : p_if_nbr        Pointer to given interface number if any.
...
*                                      NetIPv6_IsAddrMcast()
*
* Description : Validate an IPv6 address as a multicast address.
*
* Argument(s) : p_addr       Pointer to IPv6 address to validate.
*
...
*               DEF_NO,  otherwise.
*
* Note(s)     : (1) Refer to RFC #4291, Section 2.6 for the Multicast IPv6 address format.
*********************************************************************************************************
*/
...
*
* Description : Validate that an IPv6 address is the solicited node multicast address associated with an
*               IPv6 unicast address.
*
* Argument(s) : p_addr           Pointer to IPv6 address to validate.
*
*               p_addr_input     Pointer to input IPv6 unicast address.
*
* Return(s)   : DEF_YES, if IPv6 address is a solicited node multicast IPv6 address.
*               DEF_NO,  otherwise.
*
* Note(s)     : (1) RFC #4291 Section 2.7.1 specifies that "Solicited-Node multicast address are computed
...
*
*               (2) [INTERNAL] As the 24 low-order 24 bits of the input address MUST be compared, the length
...
*
*               id_type     IPv6 interface identifier type:
*
...
*
*               (2) [INTENRAL] RFC #4291, Section 2.5.1 states that:
*
...
*
*               p_addr_prefix   Pointer to variable that will receive the created IPv6 addr.
*
...
*
* Description : Get the IPv6 address masked with the prefix length.
*
* Argument(s) : p_addr      Pointer to IPv6 address.
*
...
*
* Argument(s) : p_mem_seg   Pointer to memory segment to use to allocate IPv6 objects.
...
*
* Description : Reset a configured address.
*
* Argument(s) : if_nbr  Interface number on which the address is configured.
*
*               p_addr  Pointer to the IPv6 address to reset.
*
*               p_err   Error pointer.
*
* Return(s)   : none.
...
*                   MUST be called with the global network lock already acquired.
*
*               (2) This function assumes that the TCP-IP stack does not permit two interfaces to setup
*                   the same address.
*********************************************************************************************************
*/
...
*
* Argument(s) : p_addr1         Pointer to IPv6 address mask.
*               -------------   Argument validated by caller(s).
*
*               p_addr2         Pointer to the masked IPv6 address.
*               -------------   Argument validated by caller(s).
*
* Return(s)   : DEF_OK,   if the address and the prefix mask are matching.
*               DEF_FAIL, otherwise.
...
*
* Description : Get IPv6 address object for an interface's configured IPv6 address.
*
* Argument(s) : p_addr      Pointer to configured IPv6 host address to get the interface number & IPv6
*                               addresses structure (see Note #1).
*
...
*
*               p_ext_hdr_list  Pointer to IPv6 extension headers list.
*
...
*
* Description : Stop without disabling the IPv6 Auto-Cfg process.
*
* Argument(s) : if_nbr  Interface number on which to stop the Auto-cfg process.
*
*               p_err   Error pointer.
*
* Return(s)   : none.
                                                              ...
                                                              RTOS_ERR           *p_err)
{
    ...
    const  NET_IPv6_ADDR_OBJ    *p_addr_sel;
                                          ...
                                          RTOS_ERR       *p_err)
{
    ...
    if (frag != DEF_YES) {                                      /* If pkt NOT a frag, ...                               */
       *p_is_frag = DEF_NO;
        p_frag    = p_buf;                                      /* ... rtn non-frag'd datagram (see Note #3b1B).        */
        goto exit;
    }
...
*
* Description : Handle Router Solicitation transmission timeout and retries.
*
* Argument(s) : p_obj   Pointer to a NET_IF object.
*
* Return(s)   : none.
*
* Note(s)     : none.
