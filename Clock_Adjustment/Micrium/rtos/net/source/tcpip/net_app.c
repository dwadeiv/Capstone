...
*                                          NetApp_SockOpen()
*
* Description : Opens an application socket, with error handling.
*
...
*
* Note(s)     : (1) Socket arguments and/or operations are validated in network socket handler functions.
*                   Some arguments validated only if the validation code is enabled
*                   (i.e. RTOS_ARG_CHK_EXT_EN is DEF_ENABLED in 'rtos_cfg.h').
*
*               (2) If a non-zero number of retries is requested, a non-zero time delay SHOULD also
...
*                                          NetApp_SockClose()
*
* Description : (1) Closes an application socket, with error handling :
*
...
*               DEF_FAIL, otherwise.
*
* Note(s)     : (2) Socket arguments and/or operations are validated in network socket handler functions.
...
*                                          NetApp_SockBind()
*
* Description : Binds an application socket to a local address, with error handling.
*
* Argument(s) : sock_id         Socket descriptor/handle identifier of the application to which to bind
*                               the socket.
*
...
*
*               time_dly_ms     Transitory socket bind delay value, in milliseconds.
*
*               p_err           Pointer to the variable that will receive one of the following
*                               error code(s) from this function:
*
...
*
* Return(s)   : DEF_OK,   application socket is successfully bound to a local address.
*               DEF_FAIL, otherwise.
*
* Note(s)     : (1) Socket arguments and/or operations are validated in network socket handler functions.
*                   Some arguments are validated only if the validation code is enabled (i.e.
...
*
*               (2) (a) Socket address structure 'AddrFamily' member MUST be configured in host-order and
...
*                       to network-order.
*
*               (3) If a non-zero number of retries is requested, a non-zero time delay SHOULD also be
...
*                                          NetApp_SockConn()
*
* Description : (1) Connects an application socket to a remote address, with error handling :
*
...
*
*
* Argument(s) : sock_id         Socket descriptor/handle identifier of application socket to which to
*                               connect.
*
...
*
*               time_dly_ms     Transitory socket connect delay value, in milliseconds.
*
*               p_err           Pointer to the variable that will receive one of the following
*                               error code(s) from this function:
*
...
*                                   RTOS_ERR_NET_CONN_CLOSED_FAULT
*
* Return(s)   : DEF_OK,   application socket has successfully connected to a remote address.
*               DEF_FAIL, otherwise.
*
* Note(s)     : (2) Socket arguments and/or operations are validated in network socket handler functions.
*                   Some arguments are validated only if the validation code is enabled (i.e.
*                   RTOS_ARG_CHK_EXT_EN is DEF_ENABLED in 'rtos_cfg.h').
*
*               (3) (a) Socket address structure 'AddrFamily' member MUST be configured in host-order and
*                       MUST NOT be converted to/from network-order.
*
*                   (b) Socket address structure addresses MUST be configured/converted from host-order
*                       to network-order.
...
*                                         NetApp_SockListen()
*
* Description : Sets an application socket to listen for connection requests, with error handling.
*
...
*                                   RTOS_ERR_NET_CONN_CLOSED_FAULT
*
* Return(s)   : DEF_OK,   application socket is successfully set to listen.
*               DEF_FAIL, otherwise.
*
* Note(s)     : (1) Socket arguments and/or operations are validated in network socket handler functions.
*                   Some arguments are validated only if the validation code is enabled (i.e.
...
*                                         NetApp_SockAccept()
*
* Description : (1) Returns a new application socket accepted from a listen application socket, with
*                   error handling :
*
...
*
*               p_addr_remote   Pointer to an address buffer that receives the socket address
*                               structure.
*
*               p_addr_len      Pointer to a variable to pass the size of the socket address structure
...
*
*               time_dly_ms     Transitory socket accept delay value, in milliseconds.
*
...
*
*               (4) (a) Socket address structure 'AddrFamily' member returned in host-order and SHOULD
*                       NOT be converted to network-order.
*                   (b) Socket address structure addresses returned in network-order and SHOULD be
*                       converted from network-order to host-order.
*
*               (5) If a non-zero number of retries is requested and the global socket blocking
...
*
*               (6) [INTERNAL] Pointers to variables that return values MUST be initialized PRIOR to all
...
*
*               time_dly_ms     Transitory socket receive delay value, in milliseconds.
*
*               p_err           Pointer to the variable that will receive one of the following error
*                               code(s) from this function:
*
...
*                   RTOS_ARG_CHK_EXT_EN is DEF_ENABLED in 'rtos_cfg.h').
*
*               (3) (a) Socket address structure 'AddrFamily' member returned in host-order and SHOULD NOT
...
*                               complete datagram.
*
*                           (B) IEEE Std 1003.1, 2004 Edition, Section 'recvfrom() : DESCRIPTION'
...
*
*                       (2) If the socket's type is stream and the receive data buffer size is NOT
...
*
*               (5) (a) (1) If a non-zero number of retries is requested and one of the following
*                           conditions:
...
*
*                   (b) One or more of the following SHOULD also be requested; otherwise, all
*                       retries will most likely fail immediately since no time will have elapsed to
            ...
            case RTOS_ERR_WOULD_OVF:
                 if (rx_len > 0) {                              /* If          rx len > 0, ...                          */
                     rx_len_tot += (CPU_INT16U)rx_len;          /* ... inc tot rx len.                                  */
                                                                ...
                                                                /* .. rtn data buf ovf err (see Note #4a2).             */
                 break;
...
*
*               time_dly_ms     Transitory socket transmit delay value, in milliseconds.
*
*               p_err           Pointer to the variable that will receive one of the following error
*                               code(s) from this function:
*
...
*                       to network-order.
*
*               (4) If a non-zero number of retries is requested and the global socket blocking
*                   ('NET_SOCK_CFG_BLOCK_SEL') is configured for non-blocking operation
*                   ('NET_SOCK_BLOCK_SEL_NO_BLOCK'); then one or more of the following SHOULD also be
*                   requested; otherwise, all retries will most likely fail immediately since no time
*                   will elapse to wait for & allow socket operation(s) to successfully complete :
...
*                       (b) A non-zero time delay
*
*               (5) Datagram sockets NOT currently blocked during transmit and therefore require NO
*                   transmit timeout.
*********************************************************************************************************
*/
...
*                                        NetApp_SetSockAddr()
*
* Description : Sets up a socket address from an IPv4 or an IPv6 address.
*
                          ...
                          RTOS_ERR             *p_err)
{
    ...
    Mem_Clr(p_sock_addr, NET_SOCK_ADDR_SIZE);
...
*
* Description : (1) Connects a client to a server using its host name with a stream (TCP) socket
...
*
*
* Argument(s) : p_sock_id           Pointer to a variable that receives the socket ID opened from this
*                                   function.
*
...
*
*               p_sock_addr         Pointer to a variable that receives the socket address of the
*                                   remote host.
*
*               p_secure_cfg        Pointer to the secure configuration (TLS/SSL), if needed.
*
...
*                                       RTOS_ERR_NET_CONN_CLOSED_FAULT
*
* Return(s)   : NET_IP_ADDR_FAMILY_IPv4, if connected successfully using an IPv4 address.
*               NET_IP_ADDR_FAMILY_IPv6, if connected successfully using an IPv6 address.
*               NET_IP_ADDR_FAMILY_UNKNOWN, otherwise.
*
* Note(s)     : (2) When a host name is passed to the remote host name parameter, this function tries
...
*
*               (3) This function is in blocking mode, which means that at the end of the function, the
...
*                                   remote host.
*
*               p_is_hostname       Pointer to variable that receives the boolean to indicate if the
...
*               NET_IP_ADDR_FAMILY_UNKNOWN, otherwise.
*
* Note(s)     : (2) When a host name is passed into the remote host name parameter, this function tries
*                   to resolve the address of the remote host using DNS.
*
*                   (a) The DNS must be present and enabled in the project for the resolve to be possible.
*
...
*
* Description : (1) Connects a client to a server using an IP address (IPv4 or IPv6) with a stream socket
...
*               p_secure_cfg        Pointer to the secure configuration (TLS/SSL), if needed.
*
*               req_timeout_ms      Connection timeout in milliseconds.
*
*               p_err               Pointer to the variable that will receive one of the following error
*                                   code(s) from this function:
*
...
*
* Note(s)     : (1) This function is in blocking mode, which mean that at the end of the function, the
*                   socket will have succeeded or failed to connect to the remote host.
*********************************************************************************************************
*/
...
*
* Description : (1) Connects a client to a server using an IP address (IPv4 or IPv6) with a datagram
...
*                                       RTOS_ERR_POOL_EMPTY
*
* Return(s)   : Socket ID, if no errors,
*               NET_SOCK_ID_NONE, otherwise.
*
* Note(s)     : None.
*********************************************************************************************************
